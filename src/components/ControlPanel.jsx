    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-base)', paddingBottom: '12px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          <button className="action-sm-btn" onClick={undo} disabled={past.length === 0} title="실행 취소 (Ctrl+Z)">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M3 10h10a5 5 0 0 1 5 5v2a5 5 0 0 1-5 5H9" strokeLinecap="round"/><path d="M3 10l6-6M3 10l6 6" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
          <button className="action-sm-btn" onClick={redo} disabled={future.length === 0} title="다시 실행 (Ctrl+Y)">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21 10H11a5 5 0 0 0-5 5v2a5 5 0 0 0 5 5h4" strokeLinecap="round"/><path d="M21 10l-6-6M21 10l-6 6" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        </div>
      </div>
      ...
      <ul className="template-list" style={{ marginBottom: '20px' }}>
        {layers.map((layer) => (
          <li key={layer.id} className="template-item" 
            style={{ borderColor: activeLayerId === layer.id ? 'var(--text-secondary)' : 'var(--border-base)', backgroundColor: activeLayerId === layer.id ? 'var(--bg-surface-hover)' : 'transparent', cursor: 'pointer' }}
      ...
      <ul className="template-list" style={{ marginBottom: '24px' }}>
        {stickers.map((sticker, index) => (
          <li key={sticker.id} className="template-item" 
            style={{ borderColor: activeStickerId === sticker.id ? 'var(--text-secondary)' : 'var(--border-base)', backgroundColor: activeStickerId === sticker.id ? 'var(--bg-surface-hover)' : 'transparent', cursor: 'pointer' }}
      ...
      {(activeLayer || activeSticker) ? (
        <div style={{ background: 'var(--bg-canvas)', padding: '16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-base)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <button className="action-sm-btn" onClick={() => activeLayer ? reorderLayer(activeLayer.id, 'down') : reorderSticker(activeSticker.id, 'down')} title="뒤로 보내기" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M19 12l-7 7-7-7" strokeLinecap="round" strokeLinejoin="round"/></svg>
                뒤로
              </button>
              <button className="action-sm-btn" onClick={() => activeLayer ? reorderLayer(activeLayer.id, 'up') : reorderSticker(activeSticker.id, 'up')} title="앞으로 가져오기" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <div className="control-group" style={{ marginTop: '8px', padding: '12px', background: 'var(--bg-surface)', border: '1px solid var(--border-base)', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {currentGradientColors.map((color, index) => (
                      <input key={index} type="color" className="control-input" style={{ padding: '0', height: '32px', width: '40px', border: 'none' }} value={color} onChange={(e) => handleGradientChange(index, e.target.value)} title={`색상 ${index + 1}`} />
                    ))}
                  </div>
                </div>
              )}
